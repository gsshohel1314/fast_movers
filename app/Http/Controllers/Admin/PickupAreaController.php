...
class PickupAreaController extends Controller
{
    public function index()
    {
        $datas = PickupArea::where('status', 1)->orderBy('id','DESC')->get();
        return view('admin.pickup_area.all',compact('datas'));
    }

    public function create($id = NULL){
        $id=PickupArea::find($id);
        return view('admin.pickup_area.add',compact('id'));
    }
    ...
    public function store(Request $request){
        ...
        if(isset($request->id)){
            PickupArea::where('id',$request->id)->update($data);
    ...
    public function softdelete(){
        $id=$_POST['modal_id'];
        $soft=PickupArea::where('status',1)->where('id',$id)->update([
            'status'=>0,
        ]);
    ...
    public function restore(){
        $id=$_POST['modal_id'];
        $restore=PickupArea::where('status',0)->where('id',$id)->update([
            'status'=>1,
        ]);
    ...
    public function delete(){
        $id=$_POST['modal_id'];
        $del=PickupArea::where('status',0)->where('id',$id)->first();
        $del->delete();
